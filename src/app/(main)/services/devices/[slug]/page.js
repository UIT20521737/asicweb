...
async function getDeviceDetail(slugOrId) {
  try {
    const res = await fetch(`${host}/api/devices/${slugOrId}`, { cache: 'no-store' });
...
function getImageUrl(thumbnail) {
  if (!thumbnail) return "/placeholder-device.jpg";
...
export default async function DeviceDetailPage({ params }) {
  ...
  const item = await getDeviceDetail(slug);

  if (!item) notFound();

  const { processedContent, headings } = processContentWithHeadings(item.content || item.description);
  const hasHeadings = headings.length > 0;
  const mainImageUrl = getImageUrl(item.thumbnail);

  return (
    <div className="bg-white dark:bg-slate-950 min-h-screen pb-20 font-sans text-slate-900 dark:text-slate-100">
      
      {/* --- HEADER SECTION: THUMBNAIL TRÁI - TIÊU ĐỀ PHẢI --- */}
      <header className="pt-10 pb-10 bg-slate-50/50 dark:bg-slate-900/20 border-b border-slate-100 dark:border-slate-800">
        <div className="container mx-auto px-4 max-w-7xl">
          <div className="flex flex-col md:flex-row items-center gap-8 md:gap-12">
            
            {/* Thumbnail bên trái */}
            <div className="w-full md:w-60 lg:w-72 shrink-0">
              <div className="relative aspect-[4/3] overflow-hidden rounded-2xl shadow-sm border border-white dark:border-slate-800 bg-white dark:bg-slate-950 flex items-center justify-center">
                <Image 
                  src={mainImageUrl} 
                  alt={item.deviceName} 
                  fill 
            ...
            <div className="flex-1 text-center md:text-left">
              <nav className="mb-4 flex items-center justify-center md:justify-start space-x-2 text-[10px] font-bold uppercase tracking-widest text-slate-400">
                ...
                <span>/</span>
                <span className="text-slate-900 dark:text-slate-100 truncate max-w-xs">{item.deviceName}</span>
              </nav>
              
              <div className="flex items-center justify-center md:justify-start gap-3 mb-4">
                 <span className="px-2.5 py-0.5 rounded-full text-[8px] font-black uppercase tracking-widest border border-emerald-500/20 bg-emerald-500/5 text-emerald-600">
                   {item.brand || 'Equipment'}
                 </span>
                 <div className="flex items-center gap-1.5 ml-2">
                    <span className={`w-1.5 h-1.5 rounded-full ${item.status === 'available' ? 'bg-emerald-500' : 'bg-amber-500'}`}></span>
                    <span className="text-[9px] font-bold uppercase tracking-wider text-slate-500">{item.status || 'Active'}</span>
                 </div>
              </div>
      ...
      <div className="container mx-auto px-4 mt-12 max-w-7xl">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-16">
          ...
          {/* Cột trái: Nội dung chính (Rộng hơn - 9 cột) */}
          ...
          {/* Cột phải: Sidebar Mục lục (Hẹp lại - 3 cột) */}
            ...
            <div className="sticky top-24">
              {hasHeadings && (
                <div className="bg-white dark:bg-slate-900 rounded-2xl p-5 border border-slate-100 dark:border-slate-800 shadow-sm">
                  ...
                  {/* Cỡ chữ mục lục nhỏ lại một chút để cân đối với cột hẹp */}
